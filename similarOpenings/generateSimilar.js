 ...
 */
export function findSimilarPositions([pos, wb, move], posArray) {
  return posArray.filter(([pos2, wb2, move2]) => {
    return move === move2 && wb === wb2 && 
...
const book = {...A.json, ...B.json, ...C.json, ...D.json, ...E.json, ...IN.json}
...
positions.forEach((pos) => {
    ...
    similarPositions[pos]??=[]
    similarPositions[pos].push(similar.map(s => s[0]))
...
fs.writeFileSync('./similar.json', JSON.stringify(similarPositions, null, 2))
