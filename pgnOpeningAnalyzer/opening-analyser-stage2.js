...
class OpeningAnalyserStage2 {
    constructor(inputFile = 'output/analysis.json', outputFile = 'output/candidates.json') {
    ...
    async loadAnalysis() {
        try {
            ...
            console.error(`Failed to load analysis from ${this.inputFile}:`, error.message);
            return null;
        }
    }
    ...
    flattenAndSort(analysisData) {
        ...
        for (const [opening, variations] of Object.entries(analysisData)) {
            for (const [variation, subvariations] of Object.entries(variations)) {
                for (const [subvariation, data] of Object.entries(subvariations)) {
                    ...
                    try {
                        ...
                        console.warn(`Failed to load PGN "${data.moveList}": ${error.message}`);
                    }
                    
                    const record = {
                        opening,
                        variation: variation === "_" ? undefined : variation,
                        subvariation: subvariation === "_" ? undefined : subvariation,
                        ...data,
                        fullFEN
                    };
    ...
    async processOpenings() {
        ...
        for (const opening of sortedOpenings) {
            ...
            if (processed % 10 === 0 || processed === sortedOpenings.length) {
                process.stdout.write(`\rProcessed: ${processed}/${sortedOpenings.length} | Found: ${foundOpenings} | Candidates: ${candidates.length}`);
            }
        }
        ...
        // Save candidates
        try {
            await fs.writeFile(this.outputFile, JSON.stringify(candidates, null, 2));
        ...
        console.log(`Coverage: ${((foundOpenings / sortedOpenings.length) * 100).toFixed(1)}%`);
